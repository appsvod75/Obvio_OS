                ...
                <>
                    ...
                    <div className="flex-1 overflow-y-auto">
                        <div className="grid grid-cols-1 sm:grid-cols-12 gap-[clamp(6px,1.5vmin,24px)]">
                            ...
                            {/* Right column */}
                            <div className="sm:col-span-12 lg:col-span-3 grid grid-rows-2 gap-[clamp(6px,1.5vmin,24px)]">
                                <div className="bg-white border border-rose-border rounded-[clamp(12px,3vmin,24px)] p-[clamp(6px,1.5vmin,16px)] shadow-xl flex flex-col gap-[clamp(4px,1vmin,8px)]">
                                    <h3 className="text-rose-500 font-black text-[clamp(8px,1.5vmin,9px)] uppercase tracking-[0.3em] flex items-center gap-2"><Receipt size={10} className="text-rose-500" /> Acciones</h3>
                                    <div className="flex flex-col gap-[clamp(4px,1vmin,8px)] flex-1 justify-center">
                                        <button onClick={() => setShowReportModal(true)} className="w-full bg-rose-palo hover:bg-rose-palo-dark text-white py-[clamp(6px,1.5vmin,10px)] rounded-xl font-black uppercase text-[clamp(9px,2vmin,10px)] shadow-lg active:scale-95 transition-all flex items-center justify-center gap-1"><Receipt size={14} /> Ticket</button>
                                        <button onClick={() => printReceipt('printable-report-z')} className="w-full bg-rose-muted text-rose-500 py-[clamp(6px,1.5vmin,10px)] rounded-xl font-black uppercase text-[clamp(9px,2vmin,10px)] shadow-xl active:scale-95 transition-all flex items-center justify-center gap-1"><Printer size={14} /> Imprimir</button>
                                    </div>
                                </div>
                                <div className="bg-white border border-rose-border rounded-[clamp(12px,3vmin,24px)] p-[clamp(6px,1.5vmin,16px)] shadow-xl flex flex-col gap-[clamp(4px,1vmin,8px)]">
                                    <h3 className="text-rose-500 font-black text-[clamp(8px,1.5vmin,9px)] uppercase tracking-[0.3em] flex items-center gap-2"><Power size={10} className="text-rose-500" /> Cierre</h3>
                                    <div className="flex flex-col gap-[clamp(4px,1vmin,8px)] flex-1 justify-center">
                                        <button className="w-full bg-rose-200 text-rose-700 py-[clamp(6px,1.5vmin,10px)] rounded-xl font-black uppercase text-[clamp(9px,2vmin,10px)] shadow-xl flex items-center justify-center gap-1 opacity-60 cursor-not-allowed"><Mail size={14} /> Correo</button>
                                        {!viewingClosureId ? (
                                            <button onClick={() => setShowConfirmModal(true)} className="w-full bg-destructive text-white py-[clamp(6px,1.5vmin,10px)] rounded-xl font-black uppercase text-[clamp(9px,2vmin,10px)] shadow-xl flex items-center justify-center gap-1 border-b-4 border-destructive active:border-b-0 active:translate-y-1 transition-all"><Power size={14} /> Cerrar Caja</button>
                                        ) : (
                                            <div className="w-full bg-blue-900/10 border border-blue-500/20 rounded-xl flex items-center justify-center py-[clamp(6px,1.5vmin,10px)] text-[clamp(10px,2.5vmin,12px)] font-black text-blue-400 uppercase">Archivado</div>
                                        )}
                                    </div>
                                </div>
                            </div>
