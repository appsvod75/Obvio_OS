...
app.put('/api/config', (req, res) => {
  ...
  try {
    const tx = db().transaction(() => {
      db().prepare(
        "INSERT INTO app_config (id, salon_name, salon_address, salon_phone, ticket_footer, logo_url, ticker_message, ticker_speed, youtube_video_id, webhook_url, ticket_size, hidden_panels, telegram_bot_token, latitude, longitude, geofence_radius) VALUES (1,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET salon_name=excluded.salon_name, salon_address=excluded.salon_address, salon_phone=excluded.salon_phone, ticket_footer=excluded.ticket_footer, logo_url=excluded.logo_url, ticker_message=excluded.ticker_message, ticker_speed=excluded.ticker_speed, youtube_video_id=excluded.youtube_video_id, webhook_url=excluded.webhook_url, ticket_size=excluded.ticket_size, hidden_panels=excluded.hidden_panels, telegram_bot_token=excluded.telegram_bot_token, latitude=excluded.latitude, longitude=excluded.longitude, geofence_radius=excluded.geofence_radius"
      ).run(c.salonName || '', c.salonAddress || '', c.salonPhone || '', c.ticketFooter || '', c.logoUrl || '', c.tickerMessage || '', c.tickerSpeed || 20, c.youtubeVideoId || '', c.webhookUrl || '', c.ticketSize || '80mm', JSON.stringify(c.hiddenPanels || []), c.telegramBotToken || null, c.latitude?.toString() || '13.706396', c.longitude?.toString() || '-89.146180', parseInt(c.geofenceRadius || 10));
